...
export default function SearchBar({ size, showMobile = true, open, setSearchQuery, showBtn, placeholder = "Search for events" }) {
    ...
    // show the modal when searchbar is clicked on in homepage 
    useEffect(() => {
        if (open) {
            handleShow()
        }
        ...
        if (e && e.split('').length >= 3) {
            ...
            searchEvents(data)
                .then((res) => {
                    ...
                    flag(true); // show dropdown even if there are no results
                })
    ...
    return (
        <Fragment>
            ...
            <div className="search-container">
                <InputGroup>
                    <Form onSubmit={handleSubmit} ref={ref} className={`w-100 d-flex position-relative ${showMobile ? 'd-none d-lg-flex' : ''} `}>
